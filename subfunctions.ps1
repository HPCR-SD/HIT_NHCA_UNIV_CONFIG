...
function New-SysopAdmin {
	$UserExist = Get-LocalUser -Name sysop
	if ($UserExist) {
		$sysopPassword = Read-Host "What is the password for SYSOP? " -AsSecureString # Improve code here by not requiring variable (Will go without variable but adds several extra lines of text) - Variable maybe helpful for setup of VPN
		Set-LocalUser -Name sysop -Password $sysopPassword
		# Makes sure user is setup properly
	}

	elseif (!$UserExist) {
		$sysopPassword = Read-Host "What will the password be for SYSOP? " -AsSecureString # Improve code here by not requiring variable (Will go without variable but adds several extra lines of text)
		New-LocalUser "sysop" -Password $sysopPassword -AccountNeverExpires -PasswordNeverExpires
		Add-LocalGroupMember -Group "Administrators" -Member "sysop"
	}
}

function New-FirstLoginUser {
	$FirstLoginPassword = Read-Host "What is the password for FirstLogin? " -AsSecureString # Improve code here by not requiring variable (Will go without variable but adds several extra lines of text)
	New-LocalUser "FirstLogin" -Password $FirstLoginPassword -AccountNeverExpires -PasswordNeverExpires
